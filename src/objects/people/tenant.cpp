...
namespace tenant {
    ...
    void Tenant::setHappiness(int newHappiness) { happiness = newHappiness; }
    void Tenant::addHappiness(int newHappiness) { happiness += newHappiness; if (happiness < 0) happiness = 0; if (happiness > 100) happiness = 100;}

    int Tenant::payRent(int rent){
        savings += monthlyIncome;
        std::random_device dev;
        std::mt19937 gen(dev());
        std::normal_distribution<> spendingDistr(spending,spending/5);
        savings -= spendingDistr(gen);
        int paid = 0;
        if (owing < savings*0.9) {
            paid += owing;
            savings -= owing;
            owing = 0;
        }
